...
class ColorService:
    ...
    def get_theme_name_for_color(r: int, g: int, b: int) -> str:
        ...
        if (
            ...
            and s > HIGH_SATURATION_THRESHOLD
            and not (160 <= h <= 260)
        ):
            return "Sand"
        ...
        for start, end, name in color_ranges:
            if start <= h < end:
                if name == "Yellow" and l > YELLOW_TO_SAND_LIGHTNESS:
                    return "Sand"
                if name == "Orange" and l < ORANGE_TO_BROWN_LIGHTNESS:
                    return "Sand"
                if name == "Blue" and l < BLUE_TO_NAVY_LIGHTNESS:
                    return "Navy"
                return name
